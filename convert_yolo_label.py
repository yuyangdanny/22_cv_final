...
def convert_annotation(image_id, paths):
    ...
    for obj in load_dict['objects']:  # load_dict['objects'] -> 目标的几何框体
        ...
        if obj_label in ['out of roi', 'ego vehicle', 'terrain', 'sky', 'road', 'ground', 'sidewalk', 'building', 'persongroup', 'static', 'vegetation', 'pole', 'traffic sign', 'cargroup', 'bicycle', 'bicyclegroup', 'traffic light', 'rider', 'fence', 'dynamic', 'license plate', 'wall', 'truck', 'parking', 'caravan', 'bus', 'motorcycle', 'polegroup', 'train', 'trailer', 'bridge', 'motorcyclegroup', 'tunnel', 'guard rail', 'truckgroup', 'rail track', 'ridergroup', 'rectification border']:  # 直接跳过这两种类型 注意测试集里只有这两种类型 跳过的话测试集合里将为空的标签
            continue
...
if __name__ == '__main__':
    ...
    root_dir = '/media/public_dataset/CityScape/'
    image_dir = root_dir + 'leftImg8bit_trainvaltest/leftImg8bit'
    label_dir = root_dir + 'gtFine_trainvaltest/gtFine'
    label_output_dir = './new_labels'
    label_map = {}
    ...
    with open(label_output_dir + '/' + 'classes.txt', 'w') as f:  # 写出类别总表
        for k, v in label_map.items():
            f.write(f'{k}\n')
# https://blog.csdn.net/Shenpibaipao/article/details/111240711