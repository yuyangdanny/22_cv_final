    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if random.random() < 0.8:
                img, labels, iffoggy = load_mosaic(self, index)
            else:
                img, labels, iffoggy = load_mosaic9(self, index)
            shapes = None

            # MixUp https://arxiv.org/pdf/1710.09412.pdf
            if random.random() < hyp['mixup']:
                if random.random() < 0.8:
                    img2, labels2, iffoggy2 = load_mosaic(self, random.randint(0, len(self.labels) - 1))
                else:
                    img2, labels2, iffoggy2 = load_mosaic9(self, random.randint(0, len(self.labels) - 1))
            ...
            # Load image
            img, (h0, w0), (h, w), iffoggy = load_image(self, index)
        ...
        if self.augment and iffoggy==1:
            ...
            # Gaussian Blur
            if random.random() < hyp['gaussian_blur']:
                transform = transforms.GaussianBlur(101, 10)
            ...
            # Pepper Noise
            if random.random() < hyp['pepper_noise']:
            ...
            if random.random() < hyp['geometric_distortions']:
...
def load_image(self, index):
    ...
    if 'foggy' in path.split('.png')[0].split('/')[-1]:
        iffoggy = 1
    else:
        iffoggy = 0
    if img is None:  # not cached
        ...
        return img, (h0, w0), img.shape[:2], iffoggy  # img, hw_original, hw_resized, iffoggy return 1 else return 0
    else:
        return self.imgs[index], self.img_hw0[index], self.img_hw[index], iffoggy  # img, hw_original, hw_resized
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        # Load image
        img, _, (h, w), iffoggy = load_image(self, index)
